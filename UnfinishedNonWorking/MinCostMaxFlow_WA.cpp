...
struct Edge {
    Edge(int u, int v, int c, int rev, int cf, int cost, bool b) : u(u), v(v), c(c), rev(rev), cost(cost), orgCost(cost), flow{ cf }, isRev{ b } {}
...
public:
    ...
    SingleShortestPath(size_t size, size_t start) : _prioQueue(_comparator()),
        ...
        _prioQueue.push(make_tuple(start, start, 0, std::numeric_limits<long long>::max(), 0), start);
    ...
    long long shortestPath(size_t goal, vector<Node> & G) {
        ...
        //First is current, 2nd is previous, 3rd is total cost;
        tuple<size_t, int, T, long long, size_t> curr;
        ...
        while (!_prioQueue.empty()) {
            ...
            path[currI] = make_pair(get<1>(curr), get<4>(curr));
            ...
            for (int i{ 0 }; i < G[currI].edges.size(); ++i) {
                ...
                if (path[neighbor].first == -1 && G[currI].edges[i].flow < G[currI].edges[i].c) { //Don't bother storing already visited
                    _prioQueue.push(
                        make_tuple(neighbor, currI, get<2>(curr) + G[currI].edges[i].cost, min(get<3>(curr), G[currI].edges[i].c - G[currI].edges[i].flow), i),
                        neighbor);
                }
            }
        }
...
private:
    struct _comparator {
        ...
        bool operator()(const tuple<size_t, int, T, long long, size_t> & lhs, const tuple<size_t, int, T, long long, size_t> & rhs)
            const {
    ...
    //First is current, 2nd is previous, 3rd is total cost;
    EditableHeap<tuple<size_t, int, T, long long, size_t>, int, _comparator> _prioQueue;
    size_t _start;
};
